...
export interface GameState {
  id: string;
  board: string[][][]; // 3x3x3, empty string for empty cells
...
export function makeMove(state: GameState, move: Move): { success: boolean; error?: string } {
  ...
  if (state.board[x][y][z]) return { success: false, error: 'Cell already occupied' };
